...
class Codec2D {
    ...
    static encode(lngLat, level = 10) {
        ...
        if (Math.abs(lngLat.latDegree) >= 88) {
            // 都变换到东经0~90°
            let part;
            let parent = { isPoint: true, latSize: 2 * 3600 };
    ...
    static encodeNPole(lngInSec, latInSec, lngN, latN, parent, n, part) {
        if (n < 0 || n > 10) {
        ...
        if (n === 0) {
            ...
            const gridCountLng = data_1.gridCountPole[n][0];
            const gridCountLat = data_1.gridCountPole[n][1];
            if (parent.isPoint) {
    ...
    static encodeFragment(level, lngCount, latCount) {
        if (level === 3 || level === 6) {
            ...
            return (lngCount.toString(16).toUpperCase() +
    ...
    static decodeN(code, n) {
        ...
        return [rowCol[0] * data_1.gridSizes1[n][0], rowCol[1] * data_1.gridSizes1[n][1]];
    }
    /**
     *
     * @param target 目标区域位置
    ...
    static getReferRange(code) {
        ...
        if (lngLatInSecond[0] >= 0) {
            ...
            eastBound =
                lngLatInSecond[0] + 8 * data_1.gridSizes1[level][0] - data_1.gridSizes1[10][0];
        }
        else {
            westBound =
                lngLatInSecond[0] - 8 * data_1.gridSizes1[level][0] + data_1.gridSizes1[10][0];
        ...
        if (lngLatInSecond[1] >= 0) {
            ...
            northBound =
                lngLatInSecond[1] + 8 * data_1.gridSizes1[level][1] - data_1.gridSizes1[10][1];
        }
        else {
            southBound =
                lngLatInSecond[1] - 8 * data_1.gridSizes1[level][1] + data_1.gridSizes1[10][1];
     ...
     * @returns string[]
     */
    static getNeighbors(code, offsets = [
        [-1, -1],
        [-1, 0],
        [-1, 1],
        [0, -1],
    ...
    static getOffset(reference, target) {
        ...
        if (reference.substring(0, data_1.codeLengthAtLevel[level - 1]) ===
            ...
            // 列差
            let lngDiff = ((tInSecond[0] - rInSecond[0]) / data_1.gridSizes1[level][0]) * lngSign;
            lngDiff = Math.floor(lngDiff);
            // 行差
            let latDiff = ((tInSecond[1] - rInSecond[1]) / data_1.gridSizes1[level][1]) * latSign;
    ...
    static getRelativeGrid(code, offsetX, offsetY) {
        ...
        if (newX >= 0 &&
            ...
            return (code.substring(0, data_1.codeLengthAtLevel[level - 1]) +
    ...
    static getCodeAtLevel(code, level) {
        ...
        return code.substring(data_1.codeLengthAtLevel[level - 1], data_1.codeLengthAtLevel[level]);
    }
    /**
     * 获取某一级别的网格的行列号
    ...
    static checkCodeFragmentRange(lng, lat, level) {
        if (level === 1) {
            if (lng > data_1.gridCount1[level][0] ||
                lng < 1 ||
                lat < 0 ||
        ...
        else if (lng > data_1.gridCount1[level][0] - 1 ||
            lng < 0 ||
            lat < 0 ||
            lat > data_1.gridCount1[level][1] - 1) {
            throw new Error("位置码错误");
    ...
    static getSecond(lngLat) {
        ...
        // 计算经度，换算成秒
        const lngInSec = (lngLat.lngDegree * 3600 +
            ((_a = lngLat.lngMinute) !== null && _a !== void 0 ? _a : (lngLat.lngMinute = 0)) * 60 +
            ((_b = lngLat.lngSecond) !== null && _b !== void 0 ? _b : (lngLat.lngSecond = 0))) *
            (((_c = lngLat.lngDirection) !== null && _c !== void 0 ? _c : "E") === "W" ? -1 : 1);
        // 计算纬度，换算成秒
        const latInSec = (lngLat.latDegree * 3600 +
            ((_d = lngLat.latMinute) !== null && _d !== void 0 ? _d : (lngLat.latMinute = 0)) * 60 +
            ((_e = lngLat.latSecond) !== null && _e !== void 0 ? _e : (lngLat.latSecond = 0))) *
            (((_f = lngLat.latDirection) !== null && _f !== void 0 ? _f : "N") === "S" ? -1 : 1);
